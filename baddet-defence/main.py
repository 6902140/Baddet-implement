...
def main():
    ...
    clean_features = [Image.open(feature_path) for feature_path in selected_features] # 隨機選取一定數量的乾淨數據
    
    for i in range(len(clean_features)):
        feature = clean_features[i].convert('RGB')
        feature = np.asarray(feature, dtype=np.float32)
        feature = feature.transpose((2, 0, 1)) # 把深度通道交換到第一個維度
        feature = preprocess(feature) # 預處理圖片調整圖像size
        clean_features[i] = feature
    print("Complete")

    print("Loading model...")
    model = FasterRCNNVGG16(n_fg_class=20) # 使用基於VGG16的FasterRCNN模型
